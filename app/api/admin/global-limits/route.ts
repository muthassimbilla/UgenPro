...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!['address_generator', 'email2name'].includes(api_type)) {
      return NextResponse.json(
        { error: 'Invalid api_type. Must be address_generator or email2name' },
        { status: 400 }
      )
    }
    ...
    if (!is_unlimited && (daily_limit < 1 || daily_limit > 10000)) {
      return NextResponse.json(
        { error: 'Daily limit must be between 1 and 10000' },
        { status: 400 }
      )
    }

    const success = await rateLimiter.setGlobalLimit(
      api_type,
      is_unlimited ? 0 : daily_limit,
      is_unlimited
    )

    if (!success) {
      return NextResponse.json(
        { error: 'Failed to update global limit' },
        { status: 500 }
      )
    }
