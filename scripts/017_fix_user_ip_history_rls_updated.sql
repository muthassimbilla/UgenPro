-- Updated RLS policy fix script that handles existing policies
-- This script will safely update RLS policies for user_ip_history table
...
drop policy if exists "Service role can update IP history for any user" on public.user_ip_history;
drop policy if exists "Service role can delete IP history for any user" on public.user_ip_history;
