...
export const ApiUsageCounter = forwardRef<ApiUsageCounterRef, ApiUsageCounterProps>(
  ({ apiType, className, showProgressBar = true, compact = false }, ref) => {
    ...
    // Update usage data after API call
      ...
      if (compact) {
        return (
          <div className={`text-xs text-gray-600 flex items-center gap-1 ${className}`}>
            ...
            <span>{apiType === "address_generator" ? "Address Generator" : "Email to Name"} - Ready</span>
      ...
      <Card className={`${getBorderColor()} ${className}`}>
        <CardContent className="p-4">
          <div className="space-y-3">
            ...
            {unlimited && <div className="text-xs text-purple-600 font-medium">You have unlimited access</div>}

            {isAtLimit && (
              <div className="text-xs text-red-600">Today's limit has been reached. Please try again tomorrow.</div>
            )}

            {isNearLimit && <div className="text-xs text-orange-600">Warning: Your limit is almost reached</div>}

            <div className="text-xs text-gray-400">Today's usage • Resets at midnight</div>
          </div>
        </CardContent>
      </Card>
    )
...
export function useApiUsageCounter(apiType: ApiType) {
  ...
  const Counter = React.useCallback(
    (props: { className?: string }) => <ApiUsageCounter ref={counterRef} apiType={apiType} {...props} />,
    [apiType],
  )
