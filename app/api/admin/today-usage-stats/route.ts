...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = {
      ...
      byApiType: {
        address_generator: {
          totalCalls: todayUsage.filter(u => u.api_type === 'address_generator').reduce((sum, u) => sum + u.daily_count, 0),
    ...
    console.error('Error fetching today\'s usage statistics:', error)
    return NextResponse.json(
      { error: 'Failed to fetch today\'s usage statistics', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
